...
```mermaid
flowchart TD
    A["Repeat V minus 1 times"] --> B["Scan every edge"]
    B --> C{"Better path found"}
    C -->|Yes| D["Update distance"]
    C -->|No| E["Keep current value"]
    D --> B
    E --> B
```
...
```java
boolean hasNegativeCycle(List<Edge> edges, long[] dist) {
    for (Edge e : edges) {
        if (dist[e.from] == INF) continue;
        if (dist[e.to] > dist[e.from] + e.cost) {
            return true;
