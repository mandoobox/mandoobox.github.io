...
```mermaid
flowchart TD
    A["For each i in 0..N-1"] --> B{"deque front out of window?<br/>(front ≤ i - K)"}
    B -->|Yes| C["pollFirst()"]
    C --> B
    B -->|No| D{"deque back ≥ arr[i]?"}
    D -->|Yes| E["pollLast()"]
    E --> D
    D -->|No| F["offerLast(i)"]
    F --> G{"i ≥ K - 1?"}
    G -->|Yes| H["result = arr[deque.peekFirst()]"]
    G -->|No| A
    H --> A
```
...
```mermaid
flowchart TD
    A["Start: offerFirst(start)"] --> B["pollFirst() = u"]
    B --> C["For each edge (v, w) from u"]
    C --> D{"dist[u] + w < dist[v]?"}
    D -->|Yes| E{"w == 0?"}
