...
flowchart TD
    subgraph "offer(x)"
        A1["Insert at last position"] --> A2["Sift Up: swap with parent while smaller"]
    end
    subgraph "poll()"
        B1["Remove root"] --> B2["Move last to root"] --> B3["Sift Down: swap with smaller child"]
    end
```
...
```mermaid
flowchart LR
    A["Stream elements"] --> B["Insert into min-heap (size K)"]
    B --> C{"heap.size > K?"}
    C -->|Yes| D["poll() removes smallest"]
    C -->|No| B
    D --> B
    B --> E["Final heap = Top K largest"]
```
...
int getMedian() {
    return maxHeap.peek(); // 홀수 개일 때 중앙값
}
```
...
graph LR
    subgraph "maxHeap (left half)"
        M1["keeps smaller half"]
    end
    subgraph "minHeap (right half)"
        M2["keeps larger half"]
    end
    M1 -- "maxHeap.peek() ≤ minHeap.peek()" --> M2
    M1 -- "median = maxHeap.peek()" --> R["Result"]
```
