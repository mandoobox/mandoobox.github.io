...
핵심: √N까지만 확인하면 된다.
코드에서는 overflow를 피하려고 `(long) i * i <= n`처럼 쓰면 안전하다.
...
구현할 때는 `i * i`가 `int` 범위를 넘을 수 있으므로
위 코드처럼 `(long) i * i`로 계산해 두면 더 안전하다.
...
- `gcd(b, M) = 1`일 때만 역원이 존재하고
- 이때는 확장 유클리드 알고리즘으로 구하는 방식이 일반적이다

코테에서는 보통 `MOD = 1_000_000_007` 같은 소수 모듈러가 자주 나오므로
페르마 소정리를 쓰는 버전을 먼저 익히면 된다.
