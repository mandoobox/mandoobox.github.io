...
단, 이 정의는 **그래프가 연결되어 있을 때**만 성립한다.
그래프가 연결되어 있지 않다면 MST는 존재하지 않고,
대신 각 연결 요소별 최소 신장 포레스트(Minimum Spanning Forest)를 구하게 된다.
...
long kruskal(int n, int[][] edges) {
    ...
    if (edgeCount != n - 1) {
        throw new IllegalArgumentException("그래프가 연결되어 있지 않아 MST가 존재하지 않습니다.");
    }
