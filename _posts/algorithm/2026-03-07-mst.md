...
```mermaid
flowchart TD
    A["Sort edges by weight"] --> B["Pick cheapest edge"]
    B --> C{"Creates cycle?<br/>(find(u) == find(v))"}
    C -->|Yes| D["Skip"]
    C -->|No| E["Add to MST, union(u,v)"]
    D --> F{"N-1 edges selected?"}
    E --> F
    F -->|No| B
...
임의의 컷(두 그룹으로 나누기)에서 가장 가벼운 간선은 반드시 MST에 포함된다.

### 2) Cycle Property

임의의 사이클에서 가장 무거운 간선은 MST에 포함되지 않는다.
