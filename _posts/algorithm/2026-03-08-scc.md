...
그래프 문제가 SCC 이후 갑자기 쉬워지는 이유가 여기 있다.
...
즉 이 사이클 위의 모든 SCC는 서로 왕복 도달 가능하다.
그런데 SCC는 "서로 왕복 도달 가능한 최대 묶음"이므로,
이들은 원래 서로 다른 SCC일 수 없다.
...
실전에서는 이 성질 하나 때문에
"원래 그래프는 복잡하지만 SCC DAG에서는 위상 정렬/DP가 된다"는 판단이 가능해진다.

### SCC 번호 순서를 너무 믿지 말기

Kosaraju 구현에서 `sccId`는 DFS가 SCC를 발견한 순서대로 붙는다.

이 순서가 압축 DAG 흐름과 어느 정도 맞아떨어지는 경우가 많지만,
문제 풀이에서는 이 번호 자체를 정렬 순서처럼 가정하지 않는 편이 안전하다.
