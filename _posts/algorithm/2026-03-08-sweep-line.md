...
예를 들어 좌표가 `1`부터 `10^9`까지 가능하지만
실제로 등장하는 시작/끝점은 `2 * N`개뿐일 수 있다.
...
즉 sweep line은 단순히 "정렬해서 본다"가 아니라
**실제로 변화가 일어나는 좌표만 본다**는 관점으로 이해하면 좋다.
...
즉 sweep line의 본질은 "정렬된 이벤트를 순서대로 처리"하는 것이고,
그 안에서 유지하는 상태는 문제마다 달라진다.
